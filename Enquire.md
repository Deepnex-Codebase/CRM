...
audit_log
Reference Tables:
source_channels, profile_types, status_types, priority_score_types, assignment_rules, automation_rules, dashboard_configs, kpi_scores, notification_templates, action_types, role_types, ,report_logs
Output: Decides which Profile (Project / Product / AMC / Info / site_visit_schedule / complaint / Job
) the enquiry converts into.
ç# ¡/ DATABASE SCHEMA TABLE
The CRM/Enquiry Management system is responsible for capturing, qualifying, assigning, and tracking enquiries from multiple channels.
Core Tables
enquiries
...
Workflow Steps:
Lead Capture
Leads come from digital channels, integrations, imports, manual entry.
Validation
Deduplication check.
Contact verification.
Classification
Enquiry type (project, product, info, AMC, complaint).
Scoring
AI assigns priority_score based on source, history, and channel.
Assignment
Auto-assign using assignment_rules or manual by manager.
Follow-up
Tasks, calls, and communication logs updated.
Conversion / Closure
If qualified: Converted → creates a profile in profile_mapping.
If unqualified: Closed (Lost/Dropped).
...
Q˙·. UI WIREFRAMES / MOCKUPS
Screens:
Enquiry Dashboard
Filter by status, channel, assigned team.
Enquiry Detail View
Shows contact info, history, communication logs.
Task Panel
Inline creation of tasks.
Activity Timeline
Chronological view of all actions.
Conversion Wizard
Steps for profile conversion.

Ç ERD DIAGRAM (Simplified)
lua CopyEdit
...
)J DEVELOPER TASK LIST (JIRA-STYLE)
Sprint 1: Core APIs
CRUD for enquiries
...
ç⏷<‘1_ ˙‘+ UI WALKTHROUGH (Screen by Screen)
Enquiry Dashboard → Search & Filters
Click Enquiry → Detail View → Contact/Status/History
Assign / Update Status → Inline Action
Convert → Launch Conversion Wizard (choose profile type)
...
/#¡ç   SUMMARY TABLE OF MODULES + FEATURES
Sub-module	Key Features
Enquiries	Capture, classify, assign Assignment Logs Auto/manual assignment history Status Logs	History of status transitions Communication Email, SMS, WhatsApp tracking Conversion	Profile creation from qualified leads
...
TECHNICAL SPEC DOCUMENT REST APIs:
/enquiries
/tasks
/status_log
/assignment
...
• ˙g●G MINDMAPS OR FLOWCHARTS
pgsql CopyEdit Lead Capture
-> Deduplication
-> Qualification
-> Assignment
-> Follow-up
...
+	+
|
| (Deduplication, Validation, AI Scoring) v
+	+
| Assignment Engine  |
+	+
|
